...
def newCake():
    ...
    if request.method == 'POST':
        newCake = Cake(
            ...
            picture=request.form['picture'], price_for_kg=request.form['price_for_kg'],
            diameter=request.form['diameter'], weight=request.form['weight'],
            price_total=request.form['price_total'], client=request.form['client'],
            sell_date=request.form['sell_date'], costs=request.form['costs'],
            proceeds=request.form['proceeds'], user_id=login_session['user_id'])
        session.add(newCake)
...
def editCake(cake_id):
    ...
    if request.method == 'POST':
        if request.form['proceeds']:
            editedCake.proceeds = request.form['proceeds']
        if request.form['costs']:
            editedCake.costs = request.form['costs']
        if request.form['sell_date']:
            editedCake.sell_date = request.form['sell_date']
        if request.form['client']:
            editedCake.client = request.form['client']
        if request.form['price_total']:
            editedCake.price_total = request.form['price_total']
        if request.form['weight']:
            editedCake.weight = request.form['weight']
        if request.form['diameter']:
            editedCake.diameter = request.form['diameter']
        if request.form['price_for_kg']:
            editedCake.price_for_kg = request.form['price_for_kg']
        if request.form['picture']:
            editedCake.picture = request.form['picture']
...
@app.route('/cake/<int:cake_id>/')
def description(cake_id):
    cake = session.query(Cake).filter_by(id=cake_id).one()    
    return render_template('description.html', cake=cake)
...
def newElement(cake_id):
    ...
    if request.method == 'POST':
        newElement = Element(name=request.form['name'], price_per_kg=request.form[
            'price_per_kg'], weight=request.form['weight'],
            price_element=request.form['price_element'], cake_id=cake_id, user_id=cake.user_id)
...
def editElement(cake_id, element_id):
    ...
    if request.method == 'POST':
        if request.form['price_element']:
            editedElement.price_element = request.form['price_element']
        if request.form['weight']:
            editedElement.weight = request.form['weight']
        if request.form['name']:
            editedElement.name = request.form['name']
        if request.form['price_per_kg']:
            editedElement.price_per_kg = request.form['price_per_kg']
